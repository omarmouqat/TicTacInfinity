  ...
  return (
    <div className="w-full h-dvh overflow-hidden">
      ...
      <div className="mx-auto w-8/10 h-40/100 flex-4 flex flex-row justify-evenly items-center gap-[3dvw]">
        <button data-tooltip-id='tooltip' data-tooltip-content="Play With Computer." onClick={()=>{setMode(1);}} className={`transition-colors duration-300 flex-1 h-1/2 w-1/2 stroke-[0.4] ${(getMode()===1)?"text-green-500":"text-black"}`}><Cpu className="flex-1 h-full w-full stroke-[0.4]"/></button>
        <button data-tooltip-id='tooltip' data-tooltip-content="Play With Friends." onClick={()=>{setMode(2);}} className={`transition-colors duration-300 flex-1 h-1/2 w-1/2 stroke-[0.4] ${(getMode()===2)?"text-green-500":"text-black"}`}><Network className="flex-1 h-full w-full stroke-[0.4]"/></button>
        <button data-tooltip-id='tooltip' data-tooltip-content="Play infinitely with friends." onClick={()=>{setMode(3);}} className={`transition-colors duration-300 flex-1 h-1/2 w-1/2 stroke-[0.4] ${(getMode()===3)?"text-green-500":"text-black"}`}><Infinity className="flex-1 h-full w-full stroke-[0.4]"/></button>
      </div>
